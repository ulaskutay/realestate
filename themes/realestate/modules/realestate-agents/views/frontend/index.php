<?php
/**
 * Real Estate Agents - Danışmanlar Listesi Sayfası
 */

// Header'ı render et
get_header([
    'title' => __('Emlak Danışmanlarımız') . ' - ' . get_option('site_name', ''),
    ...
    <div class="container mx-auto px-4 lg:px-6">
        <div class="max-w-4xl">
            ...
            <p class="text-xl text-blue-100">
                <?php echo esc_html(__('Profesyonel ekibimizle tanışın')); ?>
            </p>
        </div>
    </div>
</section>
    ...
    <div class="container mx-auto px-4 lg:px-6">
        <?php if (empty($agents)): ?>
            <div class="text-center py-16">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-2"><?php echo esc_html(__('Henüz danışman eklenmemiş')); ?></h3>
                <p class="text-gray-600"><?php echo esc_html(__('Yakında danışmanlarımız burada görünecek.')); ?></p>
            </div>
        <?php else: ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                <?php foreach ($agents as $agent): 
                    $agentUrl = function_exists('localized_url') ? localized_url('/danisman/' . $agent['slug']) : site_url('/danisman/' . $agent['slug']);
                        ...
                        <div class="relative h-72 overflow-hidden">
                            ...
                            <?php if (!empty($agent['experience_years'])): ?>
                                <div class="absolute bottom-4 left-4 z-10">
                                    <div class="px-4 py-2 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg">
                                        <div class="flex items-center gap-2">
                                            <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <div>
                                                <div class="text-lg font-bold text-gray-900"><?php echo esc_html($agent['experience_years']); ?></div>
                                                <div class="text-xs text-gray-600"><?php echo esc_html(__('Yıl')); ?></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                <h3 class="text-2xl font-bold text-gray-900 mb-1 group-hover:text-blue-600 transition-colors">
                                    <?php echo esc_html($agentName); ?>
                                </h3>
                                <?php if (!empty($agent['specializations'])): ?>
                                    <p class="text-sm text-blue-600 font-medium">
                                        <?php echo esc_html(mb_substr($agent['specializations'], 0, 60)); ?>
                                        <?php echo mb_strlen($agent['specializations']) > 60 ? '...' : ''; ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                            ...
                            <?php if (!empty($agent['bio'])): ?>
                                <p class="text-sm text-gray-600 mb-5 line-clamp-2 leading-relaxed">
                                    <?php echo esc_html(mb_substr(strip_tags($agent['bio']), 0, 120)); ?>
                                    <?php echo mb_strlen(strip_tags($agent['bio'])) > 120 ? '...' : ''; ?>
                                </p>
                            <?php endif; ?>
                                ...
                                <?php if (!empty($agent['phone'])): ?>
                                    <a href="tel:<?php echo esc_attr($agent['phone']); ?>" 
                                       class="flex items-center gap-3 text-sm text-gray-700 hover:text-blue-600 transition-colors group/contact">
                                        ...
                                        <span class="font-medium"><?php echo esc_html($agent['phone']); ?></span>
                                    </a>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($agent['email'])): ?>
                                    <a href="mailto:<?php echo esc_attr($agent['email']); ?>" 
                                       class="flex items-center gap-3 text-sm text-gray-700 hover:text-blue-600 transition-colors group/contact">
                                        ...
                                        <span class="font-medium truncate"><?php echo esc_html($agent['email']); ?></span>
                                    </a>
                                <?php endif; ?>
                            </div>
                            ...
                            <a href="<?php echo esc_url($agentUrl); ?>" 
                               class="block w-full text-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 font-semibold shadow-md hover:shadow-lg transform hover:scale-[1.02] flex items-center justify-center gap-2">
