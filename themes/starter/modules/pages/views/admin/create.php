                ...
                <form action="<?php echo admin_url('module/pages/store'); ?>" method="POST" id="page-form">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border-2 border-blue-200 dark:border-blue-900 p-8 mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <!-- İletişim Şablonu -->
                            <label class="template-option cursor-pointer">
                                <input type="radio" name="custom_fields[page_template]" value="contact" class="hidden template-radio">
                                <div class="template-card bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl p-6 transition-all hover:border-blue-500 hover:shadow-lg">
                                    <div class="flex items-center justify-between mb-4">
                                        <span class="material-symbols-outlined text-4xl text-green-500">mail</span>
                                        <span class="template-check hidden w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center">
                                            <span class="material-symbols-outlined text-white text-sm">check</span>
                                        </span>
                                    </div>
                                    <h4 class="font-bold text-gray-900 dark:text-white mb-2">İletişim</h4>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Form, harita, iletişim bilgileri</p>
                                </div>
                            </label>
                        </div>
                    </div>
        ...
        if (selectedTemplate === 'default') {
            ...
            document.querySelectorAll('.tab-content').forEach(content => {
                ...
                if (!allowedGroups.includes(contentGroup)) {
                    ...
                    // İzin verilen grupların içeriklerini göster (ilk aktif olanı seç)
                }
            });
            
            // İlk görünür sekmeyi aktif yap
            const visibleTabs = document.querySelectorAll('.tab-btn:not(.hidden-tab)');
            if (visibleTabs.length > 0) {
                // Önce tüm tab'ları deaktif yap
                ...
                const firstTabGroup = firstTab.dataset.tab;
...
// Tab Navigation - Event Delegation ile
const tabsContainer = document.getElementById('custom-fields-tabs');
if (tabsContainer) {
    tabsContainer.addEventListener('click', function(e) {
        const btn = e.target.closest('.tab-btn');
        if (!btn) return;
        
        // Gizli tab'a tıklanmasını engelle
