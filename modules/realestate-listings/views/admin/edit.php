    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 flex flex-col lg:ml-64">
            ...
            <div class="layout-content-container flex flex-col w-full mx-auto max-w-7xl">
                                    ...
                                    <div class="border-t border-gray-200 dark:border-gray-600 pt-5 mt-2">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                                            <div>
                                                ...
                                                <input type="text" id="latitude" name="latitude" value="<?php echo esc_attr($listing['latitude'] ?? ''); ?>" placeholder="39.1234" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                                            </div>
                                            <div>
                                                <label for="longitude" class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Boylam</label>
                                                <input type="text" id="longitude" name="longitude" value="<?php echo esc_attr($listing['longitude'] ?? ''); ?>" placeholder="35.5678" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                                            </div>
                                            <div>
                                                ...
                                                <select id="city" name="city" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                                                    ...
                                                    <?php if (!empty($iller)): foreach ($iller as $ilCode => $ilName): ?>
                                                        <option value="<?php echo esc_attr($ilName); ?>" <?php echo (isset($listing['city']) && $listing['city'] === $ilName) ? 'selected' : ''; ?>><?php echo esc_html($ilName); ?></option>
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="neighborhood" name="neighborhood" value="<?php echo esc_attr($listing['neighborhood'] ?? ''); ?>" placeholder="Mahalle / köy" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                                            </div>
                                        </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <select id="property_type" name="property_type"
                                            ...
                                            $current_type = $listing['property_type'] ?? 'house';
                                            foreach ($property_types as $val => $lbl): ?>
                                                <option value="<?php echo esc_attr($val); ?>" <?php echo $current_type === $val ? 'selected' : ''; ?>><?php echo esc_html($lbl); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select id="listing_status" name="listing_status"
                                            ...
                                            $current_status = $listing['listing_status'] ?? 'sale';
                                            foreach ($listing_statuses as $val => $lbl): ?>
                                                <option value="<?php echo esc_attr($val); ?>" <?php echo $current_status === $val ? 'selected' : ''; ?>><?php echo esc_html($lbl); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        $listing_category_ids = array_column($listing_categories ?? [], 'id');
                                        ?>
                                        <div class="flex flex-wrap gap-x-4 gap-y-2">
                                            <?php foreach ($all_categories as $cat): ?>
                                                <label class="inline-flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" name="category_ids[]" value="<?php echo (int) $cat['id']; ?>"
                                                           <?php echo in_array((int)$cat['id'], $listing_category_ids, true) ? ' checked' : ''; ?>
                ...
                <?php if (function_exists('do_action') && !empty($listing)) do_action('realestate_listing_admin_after', $listing); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Düzenleme sayfasında mevcut il varsa ilçe listesini yükle ve ilçeyi seç
    var currentCity = <?php echo json_encode($listing['city'] ?? ''); ?>;
    var currentDistrict = <?php echo json_encode($listing['district'] ?? ''); ?>;
    if (currentCity) {
        loadIlceler(currentCity, currentDistrict);
    }
