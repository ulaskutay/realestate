...
<div class="relative flex h-auto min-h-screen w-full flex-col group/design-root overflow-x-hidden">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
        <span class="material-symbols-outlined text-5xl text-gray-300 dark:text-gray-600 block mb-4">home</span>
        <p class="text-gray-500 dark:text-gray-400 font-medium mb-2">Henüz ilan eklenmemiş</p>
        <a href="<?php echo admin_url('module/realestate-listings/create'); ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 text-sm font-medium">İlk ilanı ekleyin</a>
    </div>
    <?php else: ?>
    <!-- Tüm ekranlarda kart listesi – Düzenle ve Sil her zaman görünür -->
    <div class="space-y-4">
        <?php foreach ($listings as $listing): 
            $property_types = $property_types ?? [];
            $ptLabel = $property_types[$listing['property_type']] ?? ucfirst($listing['property_type'] ?? '');
        ?>
            <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-4">
                    <?php if (!empty($listing['featured_image'])): ?>
                        <img src="<?php echo esc_url($listing['featured_image']); ?>" alt="" class="h-24 w-32 sm:h-20 sm:w-28 rounded-lg object-cover flex-shrink-0">
                    <?php endif; ?>
                    <div class="min-w-0 flex-1">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white truncate"><?php echo esc_html($listing['title']); ?></h3>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">₺<?php echo number_format($listing['price'], 0, ',', '.'); ?></span>
                            <span class="text-xs text-gray-500 dark:text-gray-400">·</span>
                            <span class="text-xs text-gray-500 dark:text-gray-400"><?php echo esc_html($ptLabel); ?></span>
                            <span class="px-2 py-0.5 text-xs rounded-full font-medium <?php echo $listing['status'] === 'published' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'; ?>">
                                <?php echo $listing['status'] === 'published' ? 'Yayında' : 'Taslak'; ?>
                            </span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100 dark:border-gray-700 sm:border-0">
                        <a href="<?php echo admin_url('module/realestate-listings/edit/' . $listing['id']); ?>" class="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors text-sm font-medium">
                        ...
                        <form action="<?php echo admin_url('module/realestate-listings/delete/' . $listing['id']); ?>" method="post" class="inline" onsubmit="return confirm('Bu ilanı silmek istediğinizden emin misiniz?');">
                            <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors text-sm font-medium">
