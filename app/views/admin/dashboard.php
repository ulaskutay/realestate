        ...
        <div class="flex-1 flex flex-col lg:ml-64">
            ...
            <main class="flex-1 p-4 sm:p-6 lg:p-10 bg-gray-50 dark:bg-[#15202b] overflow-y-auto">
                ...
                <div class="max-w-7xl mx-auto space-y-6">
                    ...
                    <?php if (!empty($useGoogleAnalytics) && $analyticsStats): ?>
                    <!-- Ziyaretçi İstatistikleri (Google Analytics) -->
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border border-purple-200 dark:border-purple-800 p-6">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <a href="https://analytics.google.com/" target="_blank" rel="noopener noreferrer" class="text-xs text-gray-500 dark:text-gray-400 hover:text-primary flex items-center gap-1">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700 min-h-[100px]">
                                <div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-blue-500 text-lg">visibility</span></div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700 min-h-[100px]">
                                <div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-green-500 text-lg">person</span></div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700 min-h-[100px]">
                                <div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-purple-500 text-lg">trending_up</span></div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700 min-h-[100px]">
                                <div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-orange-500 text-lg">schedule</span></div>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white"><?php echo gmdate("i:s", (int)$analyticsStats['avg_duration']); ?></p>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700">
                                ...
                                <div class="space-y-2">
                                    <?php foreach ($analyticsStats['top_pages'] as $page): 
                                        ...
                                        if ($pageUrl !== '' && strpos($pageUrl, 'http') !== 0) {
                                            $pageUrl = rtrim(site_url(), '/') . (strpos($pageUrl, '/') === 0 ? '' : '/') . ltrim($pageUrl, '/');
                                        }
                                        $title = $page['page_title'] ?? '';
                                        if (empty($title) || $title === 'null') {
                                            $path = is_string($pageUrl) ? parse_url($pageUrl, PHP_URL_PATH) : '';
                                            $path = trim($path ?? '', '/');
                                            $title = $path === '' ? 'Ana Sayfa' : ucwords(str_replace(['-', '_', '.html', '.php', '/'], ' ', $path));
                                        }
                                    ?>
                                    <div class="flex items-center justify-between text-xs group">
                                        <a href="<?php echo esc_url($pageUrl ?: '#'); ?>" target="_blank" rel="noopener noreferrer" class="flex-1 text-gray-700 dark:text-gray-300 hover:text-primary transition-colors truncate flex items-center gap-2" title="<?php echo esc_attr($pageUrl); ?>">
                                        ...
                                        <span class="ml-2 px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full font-medium flex-shrink-0"><?php echo number_format($page['views']); ?></span>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                                <?php else: ?>
                    ...
                    <?php elseif (!empty($useGoogleAnalytics) && !empty($gaApiError)): ?>
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800 p-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Google Analytics</h2>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Veri alınamadı: <?php echo esc_html($gaApiError); ?></p>
                                    <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">Ayarlarda GA4 Property ID ve servis hesabı JSON'unu kontrol edin.</p>
                                </div>
                            </div>
                            <div class="flex flex-wrap gap-2 shrink-0">
                                <a href="<?php echo esc_url(admin_url('settings')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors font-medium">Ayarlar</a>
                                <a href="https://analytics.google.com/" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
                    ...
                    <?php elseif (!empty($useGoogleAnalytics)): ?>
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800 p-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-lg bg-white dark:bg-gray-800 flex items-center justify-center shadow-sm">
                                ...
                                <div>
                                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Google Analytics</h2>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">İstatistikleri burada göstermek için Ayarlar'dan GA4 Property ID ve servis hesabı JSON'unu ekleyin.</p>
                                </div>
                            </div>
                            <div class="flex flex-wrap gap-2 shrink-0">
                                <a href="<?php echo esc_url(admin_url('settings')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors font-medium">Ayarlar</a>
                    ...
                    <?php elseif ($analyticsStats): ?>
                    <!-- Ziyaretçi İstatistikleri (yerel page_views) -->
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border border-purple-200 dark:border-purple-800 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                                <span class="material-symbols-outlined text-purple-600 dark:text-purple-400">analytics</span>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700">
                                ...
                                <div class="space-y-2">
                                    <?php 
                                    foreach ($analyticsStats['top_pages'] as $page): 
                                        $pageUrl = $page['page_url'] ?? '';
                                        if ($pageUrl !== '' && strpos($pageUrl, 'http') !== 0) {
                                            $pageUrl = (rtrim(site_url(), '/') . (strpos($pageUrl, '/') === 0 ? '' : '/') . $pageUrl);
                                        }
                                        $title = $page['page_title'] ?? '';
                                        if (empty($title) || $title === 'null') {
                                    ...
                                    <div class="flex items-center justify-between text-xs group">
                                        <a href="<?php echo esc_url($pageUrl); ?>" 
                                           ...
                                           title="<?php echo esc_attr($pageUrl); ?>">
                                            <span class="material-symbols-outlined text-xs text-gray-400 group-hover:text-primary">link</span>
                                            <span class="font-medium"><?php echo esc_html($title); ?></span>
                                        </a>
...
<?php if ($analyticsStats && empty($useGoogleAnalytics)): ?>
<script>
// Auto-refresh analytics (yerel page_views, her 30 saniyede)
