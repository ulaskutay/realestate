...
class Page extends Model {
    ...
    private static $customFieldDefinitions = [
        ...
        'form_id' => [
            ...
            'options' => [], // Controller tarafından doldurulacak
            'dynamic_options' => 'forms' // Bu alan controller'a formları yüklemesi gerektiğini söyler
        ],
        'form_title' => [
            'type' => 'text',
            'label' => 'Form Başlığı',
    ...
    public function updatePage($id, $data) {
        ...
        // Özel alanları kaydet
        error_log('Page Update - Custom Fields Count: ' . count($customFields));
        error_log('Page Update - Custom Fields: ' . print_r($customFields, true));
        
        if ($result && !empty($customFields)) {
            $this->saveCustomFields($id, $customFields);
            error_log('Page Update - Custom fields saved for page ID: ' . $id);
        } else {
            error_log('Page Update - Custom fields NOT saved. Result: ' . ($result ? 'true' : 'false') . ', Empty: ' . (empty($customFields) ? 'true' : 'false'));
        }
    ...
    public function updateWithVersion($id, $data, $userId = null) {
        ...
        try {
            ...
            if ($contentChanged) {
                ...
                if (array_key_exists('version', $currentPage)) {
    ...
    public function restoreVersion($versionId, $userId = null) {
        try {
            ...
            // Eski versiyonu geri yükle - temel alanlar
            $restoreData = [
                'title' => $version['title'] ?? $currentPage['title'],
            ...
            // Versiyon numarasını artır (sadece version sütunu varsa)
            if (array_key_exists('version', $currentPage)) {
                $restoreData['version'] = ($currentPage['version'] ?? 1) + 1;
            }
            
            return $this->update($pageId, $restoreData);
        } catch (Exception $e) {
            error_log('Page restore version error: ' . $e->getMessage());
            return false;
        }
    }
